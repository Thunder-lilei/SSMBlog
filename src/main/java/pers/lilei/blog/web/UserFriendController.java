...
public class UserFriendController extends BaseController{
    ...
    private Map<String,Object> getMyFriend(@RequestParam int pageNow, @RequestParam int pageSize) {
        ...
        if (user != null) {
            ...
            if (!friendIdList.isEmpty()) {
                //获取好友信息
                userBaseInfoPojoPageInfo = userService.getFriendByUserId(pageNow, pageSize, friendIdList, user.getUserId());
            }
    ...
    private Map<String,Object> getMyFriendList() {
        ...
        if (user != null) {
            ...
            if (!friendIdList.isEmpty()) {
                //获取好友信息
                userBaseInfoParamList = userService.getFriendByUserIdList(friendIdList, user.getUserId());
            }
            modelMap.put(MessageConstant.MESSAGE, MessageConstant.MESSAGE_SUCCESS);
